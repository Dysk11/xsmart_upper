...
class Visualizer:
    ...
    def render(
        ...
        fps_value: float,
        avoidance_roi_rect: tuple[int, int, int, int] | None = None,
    ...
    def _build_canvas(
        ...
        if (avoid_x1, avoid_y1, avoid_x2, avoid_y2) == (x1, y1, x2, y2):
            avoid_x1 = min(avoid_x2, avoid_x1 + 3)
            avoid_y1 = min(avoid_y2, avoid_y1 + 3)
            avoid_x2 = max(avoid_x1, avoid_x2 - 3)
            avoid_y2 = max(avoid_y1, avoid_y2 - 3)
