...
- 右侧岔路采用 `MAIN -> JUNCTION -> BRANCH -> RETURNING -> MAIN` 顺序状态机：在
  `track` mask 上提取凹口 A 点和右边界外扩 P 点，并根据双赛道行位于 A 点的上方还是
  下方区分“主路分出右支路”和“右支路汇入主路”；
- 汇入口始终用 A-P 虚拟右边界保持主路。分出口默认同样保持主路，只有
  `route_direction=right` 才会锁存进入支路，用最低有效左边界点连接 P 点；支路末端
  检出左边界断裂 B 点后，将 B 点连接到 ROI 右上角，断裂释放后恢复主路；
- 虚拟补线只修改控制使用的边界和 `filtered_mask`，不会修改模型输出的原始
  `track` mask，也不会用补出的像素抬高赛道置信度；
...
左侧；请求未完成时车辆可以继续接近岔路，但到达岔路后会以 `ROAD_SIGN_WAIT` 模式
