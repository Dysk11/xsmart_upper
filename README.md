...
- 从车体附近的前景区间取得左右种子，按相反方向使用八邻域法沿 `track` mask 向远处跟踪左右边界；跟踪使用一像素虚拟背景防止越界，真实前景触及 ROI 左右边缘时仍把 `x=0` 或 `x=width-1` 作为有效实测边界并继续向上查找；
- 根据左右边界得到原始中线，依次执行梯度限幅和五点滑动平均；
- 固定透视宽度在 ROI 顶部为 `74 px`、底部为 `250 px`，中间按纵坐标线性插值；
- 正常巡线时仅使用实测边界点计算二次拟合平均绝对残差；某侧残差超过 `3 px` 时，连续 2 帧使用另一侧边界按透视宽度重建中心线。两侧都不平滑时选择残差较小侧，差值不超过 `0.1 px` 时保留正常中线；
- 中心线优先级固定为：已确认岔路中心线 > 正常巡线平滑边界重建 > 左右边界中点；
- 岔路确认后，分别由最左边界向右、最右边界向左内推半个透视宽度，得到左/右候选中线；
- 候选距离相对透视宽度连续 5 行达到 `0.15` 时区分双线，连续 5 行低于 `0.10` 时恢复普通中线；
- 首次自动选路前，对原始左右候选线做二次拟合；平均绝对残差超过 `smoothness_residual_threshold_px`（默认 `3 px`）的候选视为不平滑并舍弃。若两侧都不平滑，则舍弃残差更大的一侧；残差差不超过 `smoothness_tie_margin_px`（默认 `0.1 px`）时回退到画面中心距离规则；
